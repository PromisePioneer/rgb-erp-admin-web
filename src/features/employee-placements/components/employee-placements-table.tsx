/**
 * Employee Placements Table Component
...
export function EmployeePlacementsTable() {
  ...
  const [editingPlacementId, setEditingPlacementId] = useState<number | undefined>(undefined)

  // Single source of truth for fetch - debounced, primitive dependencies
  useEffect(() => {
    const timer = setTimeout(() => {
      fetchPlacements({ search: filters.search, status: filters.status, page: 1, per_page: 15 })
    }, 300)
  ...
  const handlePageChange = useCallback(
    (newPage: number) => {
      ...
      fetchPlacements({ search: filters.search, status: filters.status, page: newPage, per_page: 15 })
    },
    [fetchPlacements, filters.search, filters.status, pagination.last_page]
  )
  ...
  const handleEdit = (placement: EmployeePlacement) => {
    setFormMode('edit')
  ...
  // Define columns
  const columns: DataTableColumn<EmployeePlacement>[] = [
    {
  ...
  return (
    <div className="space-y-4">
      ...
      <DataTable
        ...
        emptyMessage="No employee placements found"
        enableRowSelection
      ...
      <AlertDialog open={showDeleteConfirm} onOpenChange={setShowDeleteConfirm}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Apakah Anda yakin ingin menghapus {selectedIds.size} penempatan yang dipilih? Tindakan
      ...
      {/* Employee Placement Form Modal */}
      <EmployeePlacementsFormModal
        open={showFormModal}
        onOpenChange={setShowFormModal}
        mode={formMode}
        placementId={editingPlacementId}
      />
    </div>
  )
}
