/**
 * Product Categories Form Modal Component
...
import { useProductCategoriesStore } from '@/features/product-categories'

interface ProductCategoriesFormModalProps {
  open: boolean
...
export function ProductCategoriesFormModal({ open, onOpenChange, mode, productCategoryId }: ProductCategoriesFormModalProps) {
  const {
    selectedItem,
    isLoading,
    isSubmitting,
  ...
  useEffect(() => {
    if (mode === 'edit' && productCategoryId && open) {
      fetchById(productCategoryId)
    }
    if (mode === 'create' && open) {
      resetForm()
    }
  }, [mode, productCategoryId, open, fetchById, resetForm])
  ...
  const onSubmit = async (values: ProductCategoryFormValues) => {
    ...
    try {
      if (mode === 'create') {
        ...
        await update(productCategoryId, payload)
      ...
      <DialogContent className="max-w-md max-h-[90vh] overflow-y-auto">
        <DialogHeader className="pb-4 border-b">
          <DialogTitle className="text-xl font-semibold flex items-center gap-2">
            ...
            {mode === 'create' ? 'Add New Product Category' : 'Edit Product Category'}
          </DialogTitle>
        </DialogHeader>
        ...
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6 py-4">
          ...
          <div className="space-y-2">
            ...
            <Input
              ...
              {...form.register('name', { required: 'Nama kategori produk wajib diisi' })}
              className="h-11"
            />
