/**
 * Client Types Form Modal Component
...
interface ClientTypesFormModalProps {
  open: boolean
...
export function ClientTypesFormModal({
  open,
  onOpenChange,
  mode,
  clientTypeId,
}: ClientTypesFormModalProps) {
  const {
    selectedItem,
    isLoading,
    isSubmitting,
  ...
  useEffect(() => {
    if (mode === 'edit' && clientTypeId && open) {
      fetchById(clientTypeId)
    }
    if (mode === 'create' && open) {
      resetForm()
    }
  }, [mode, clientTypeId, open, fetchById, resetForm])
  ...
  const onSubmit = async (values: ClientTypeFormValues) => {
    const payload = {
      name: values.name,
    ...
    try {
      if (mode === 'create') {
        ...
        await update(clientTypeId, payload)
      ...
      <DialogContent className="max-w-md max-h-[90vh] overflow-y-auto">
        <DialogHeader className="pb-4 border-b">
          <DialogTitle className="text-xl font-semibold flex items-center gap-2">
            ...
            {mode === 'create' ? 'Add New Client Type' : 'Edit Client Type'}
          </DialogTitle>
        </DialogHeader>
