...
export function BanksTable() {
  ...
  const [editingBankId, setEditingBankId] = useState<number | undefined>(undefined)

  // Single source of truth for fetch - debounced, primitive dependencies
  useEffect(() => {
    const timer = setTimeout(() => {
      fetchBanks({ search: filters.search, page: 1, per_page: 15 })
    }, 300)
  ...
  const handlePageChange = useCallback((newPage: number) => {
    ...
    fetchBanks({ search: filters.search, page: newPage, per_page: 15 })
  }, [fetchBanks, filters.search, pagination.last_page])
      ...
      <AlertDialog open={showDeleteConfirm} onOpenChange={setShowDeleteConfirm}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Apakah Anda yakin ingin menghapus {selectedIds.size} bank yang dipilih? Tindakan ini tidak dapat dibatalkan.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
