...
export function ProductsImportModal({ open, onOpenChange, onSuccess }: ProductsImportModalProps) {
  ...
  useEffect(() => {
    ...
    const poll = async () => {
      try {
        const result = await productsApi.getImportStatus(currentJobId)
        ...
        if (data.status === 'completed') {
          ...
          await productsApi.clearImportStatus(currentJobId).catch(() => {})

          toast.success(
            `Berhasil import ${data.imported} produk. ${data.skipped > 0 ? `${data.skipped} di-skip.` : ''} ${data.errors > 0 ? `${data.errors} error.` : ''}`,
  ...
  const handleDownloadTemplate = () => {
    window.location.href = productsApi.getTemplateUrl()
    ...
    <Dialog open={open} onOpenChange={handleClose}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>Upload file Excel atau CSV untuk import data produk</DialogDescription>
        </DialogHeader>
