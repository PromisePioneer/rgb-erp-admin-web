...
export function WarehousesFormModal({ open, onOpenChange, mode, warehouseId }: WarehousesFormModalProps) {
  const {
    selectedItem,
    isLoading,
    isSubmitting,
  ...
  useEffect(() => {
    if (mode === 'edit' && warehouseId && open) {
      fetchById(warehouseId)
    }
    if (mode === 'create' && open) {
      resetForm()
    }
  }, [mode, warehouseId, open, fetchById, resetForm])
  ...
  const onSubmit = async (values: WarehouseFormValues) => {
    ...
    try {
      if (mode === 'create') {
        ...
        await update(warehouseId, payload)
