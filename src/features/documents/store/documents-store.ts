/**
 * Documents Store
 * Zustand state management for documents
 */
import { create } from 'zustand'
import type {
  Document,
  DocumentsFilters,
  DocumentsPagination,
  CreateDocumentPayload,
  UpdateDocumentPayload,
} from '../types/documents.types'
import { documentsApi } from '../api/documents-api'

interface DocumentsState {
  // State
  items: Document[]
  selectedItem: Document | null
  isLoading: boolean
  isSubmitting: boolean
  error: string | null
  filters: DocumentsFilters
  pagination: DocumentsPagination

  // Actions
  fetchDocuments: (params?: DocumentsFilters) => Promise<void>
  fetchById: (id: number) => Promise<void>
  create: (payload: CreateDocumentPayload) => Promise<void>
  update: (id: number, payload: UpdateDocumentPayload) => Promise<void>
  remove: (id: number) => Promise<void>
  bulkDelete: (ids: number[]) => Promise<void>
  setFilters: (filters: Partial<DocumentsFilters>) => void
  resetFilters: () => void
  resetForm: () => void
  clearError: () => void
}

const initialFilters: DocumentsFilters = {
  page: 1,
  per_page: 15,
}

const initialPagination: DocumentsPagination = {
  current_page: 1,
  per_page: 15,
  total: 0,
  last_page: 1,
}

export const useDocumentsStore = create<DocumentsState>((set, get) => ({
  // Initial state
  items: [],
  selectedItem: null,
  isLoading: false,
  isSubmitting: false,
  error: null,
  filters: initialFilters,
  pagination: initialPagination,

  // Actions
  fetchDocuments: async (params?: DocumentsFilters) => {
    set({ isLoading: true, error: null })

    try {
      const currentFilters = { ...get().filters, ...params }
      const response = await documentsApi.getList(currentFilters)

      set({
        items: response.data,
        pagination: response.meta ?? get().pagination,
        isLoading: false,
      })
    } catch (error) {
      const message =
        error instanceof Error ? error.message : 'Failed to fetch documents'
      set({ error: message, isLoading: false })
    }
  },

  fetchById: async (id: number) => {
    set({ isLoading: true, error: null, selectedItem: null })

    try {
      const response = await documentsApi.getById(id)
      set({
        selectedItem: response.data,
        isLoading: false,
      })
    } catch (error) {
      const message =
        error instanceof Error ? error.message : 'Failed to fetch document'
      set({ error: message, isLoading: false })
    }
  },

  create: async (payload: CreateDocumentPayload) => {
    set({ isSubmitting: true, error: null })

    try {
      await documentsApi.create(payload)
      set({ isSubmitting: false })
      // Refresh the list
      await get().fetchDocuments()
    } catch (error) {
      const message =
        error instanceof Error ? error.message : 'Failed to create document'
      set({ error: message, isSubmitting: false })
      throw error
    }
  },

  update: async (id: number, payload: UpdateDocumentPayload) => {
    set({ isSubmitting: true, error: null })

    try {
      await documentsApi.update(id, payload)
      set({ isSubmitting: false })
      // Refresh the list
      await get().fetchDocuments()
    } catch (error) {
      const message =
        error instanceof Error ? error.message : 'Failed to update document'
      set({ error: message, isSubmitting: false })
      throw error
    }
  },

  remove: async (id: number) => {
    set({ isSubmitting: true, error: null })

    try {
      await documentsApi.delete(id)
      set({ isSubmitting: false })
      // Refresh the list
      await get().fetchDocuments()
    } catch (error) {
      const message =
        error instanceof Error ? error.message : 'Failed to delete document'
      set({ error: message, isSubmitting: false })
      throw error
    }
  },

  bulkDelete: async (ids: number[]) => {
    set({ isSubmitting: true, error: null })

    try {
      await documentsApi.bulkDelete(ids)
      set({ isSubmitting: false })
      // Refresh the list
      await get().fetchDocuments()
    } catch (error) {
      const message =
        error instanceof Error ? error.message : 'Failed to delete documents'
      set({ error: message, isSubmitting: false })
      throw error
    }
  },

  setFilters: (newFilters: Partial<DocumentsFilters>) => {
    const updatedFilters = { ...get().filters, ...newFilters }
    // Reset to page 1 when changing filters (except page itself)
    if (!('page' in newFilters)) {
      updatedFilters.page = 1
    }
    set({ filters: updatedFilters })
  },

  resetFilters: () => {
    set({ filters: initialFilters })
  },

  resetForm: () => {
    set({
      selectedItem: null,
      error: null,
    })
  },


  clearError: () => {
    set({ error: null })
  },
}))
