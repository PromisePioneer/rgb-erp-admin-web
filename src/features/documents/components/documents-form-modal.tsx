/**
 * Documents Form Modal Component
...
interface DocumentsFormModalProps {
  open: boolean
...
export function DocumentsFormModal({ open, onOpenChange, mode, documentId }: DocumentsFormModalProps) {
  const {
    selectedItem,
    isLoading,
    isSubmitting,
  ...
  useEffect(() => {
    if (mode === 'edit' && documentId && open) {
      fetchById(documentId)
    }
    if (mode === 'create' && open) {
      resetForm()
    }
  }, [mode, documentId, open, fetchById, resetForm])
        ...
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6 py-4">
          ...
          <div className="space-y-2">
            ...
            <Input
              ...
              {...form.register('name', { required: 'Nama dokumen wajib diisi' })}
              className="h-11"
            />
